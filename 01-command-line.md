...
**Instructions**  
To follow along copy and paste ```wget https://github.com/andrewquitadamo/bioinformatics-skills/archive/master.zip && unzip master.zip && cd bioinformatics-skills-master/data/01-command-line/``` into your terminal.
...
The results should look something like ```/Users/andrewquitadamo/bioinf-ds/bioinformatics-skills-master/data/01-command-line```
###ls  
...
###Tab-completion
Tab completion is your friend. You can hit tab to fill in the rest of a command, filename or directory.  
**Instructions**  
...
```  
**Instructions**  
Use ```cp``` to copy SV_gene_sample to ```results```.  
...
```  
**Instructions**  
Use ```head``` do display the first 20 lines of ```SV_gene_sample```.
###>
...
```  
**Instructions**  
Use ```tail``` to print out the last 10 lines of ```SV_gene_sample``` to a new file called ```last_10_SV```.
###cat  
...
###column
Can be used to nicely display delimited files:  
```
column -t filename
```  
```
